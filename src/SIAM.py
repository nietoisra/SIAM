...
def modulo_comparativo_alzheimer():
    ...
    if st.session_state.healthy_avg and st.session_state.alzheimer_data is not None:
        ...
        if st.button("🔄 Mostrar/Ocultar Principales Diferencias"):
        ...
        if st.session_state.show_diff:
            ...
            #st.markdown("### 🔍 Principales diferencias observadas")
        ...
        # Inicializar el estado del botón si no existe
        if "show_graf" not in st.session_state:
            st.session_state.show_graf = False

        # Botón para alternar visibilidad
        if st.button("🔄 Mostrar/Ocultar Comparación de Señales"):
            st.session_state.show_graf = not st.session_state.show_graf

        # Visualización condicional basada en el estado
        if st.session_state.show_graf:
            st.subheader("📈 Comparación de Señales")
            if len(st.session_state.healthy_data) > 0 and st.session_state.alzheimer_data is not None:
...
def main():
    ...
    with st.sidebar:
        st.header("🔧 Selecciona un Módulo")
    ...
    elif modulo == "🏃‍♂️ Análisis de Marcha + Riesgo de Caída":
        modulo_analisis_marcha_y_riesgo()
...
@st.cache_resource
def cargar_modelo():
    modelo_path = "models/modelo_clasificacion_marcha.pkl"
    le_path = "models/label_encoder_marcha.pkl"
    try:
...
def text_to_pdf(report_text):
    ...
    for line in report_text.split('\n'):
        pdf.cell(w=0, h=5, text=line, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
##########################################################################################################
def modulo_analisis_marcha_y_riesgo():
    st.header("🏥 Análisis de Marcha y Riesgo de Caída")
    ...
    if uploaded_file is not None:
        try:
            ...
            # --- ANÁLISIS DE MARCHA (PRIMERA PESTAÑA) ---
            ...
            # --- ANÁLISIS DE RIESGO DE CAÍDA (SEGUNDA PESTAÑA O CONTINUACIÓN) ---
            st.subheader("⚠️ Evaluación de Riesgo de Caída")
            metrics = calculate_fall_risk_metrics(data)
            ...
            # Gráfico de movimientos súbitos
            if "show_sudden_movements" not in st.session_state:
                st.session_state.show_sudden_movements = True

            if st.button("🔄 Mostrar/Ocultar Gráfico de Movimientos Súbitos"):
                st.session_state.show_sudden_movements = not st.session_state.show_sudden_movements

            if st.session_state.show_sudden_movements:
                fig_line = go.Figure()
                svm = metrics['svm']['data']
                acc_peaks = metrics['suddenMovements']['acc_peaks']
                ...
                fig_line.add_trace(go.Scatter(
                    x=data['Tiempo'].iloc[acc_peaks],
                    y=svm.iloc[acc_peaks],
                    mode='markers',
                ...
                st.info("El gráfico está oculto. Haz clic en 'Mostrar/Ocultar Gráfico' para visualizarlo.")

            st.subheader("📋 Recomendaciones")
            if risk['level'] == 'BAJO':
                st.success("✅ Riesgo bajo. Patrón de marcha aceptable.")
                st.markdown("""
                - Mantén una rutina de ejercicio físico suave (como caminar o estiramientos).
                - Realiza revisiones periódicas del equilibrio y marcha.
                - Usa calzado cómodo y adecuado.
                - Asegura tu entorno (iluminación, pisos seguros).
                """)
            elif risk['level'] == 'MODERADO':
                st.warning("🟡 Riesgo moderado. Algunas alteraciones detectadas.")
                st.markdown("""
                - Realiza ejercicios enfocados en equilibrio (ej.: Tai Chi).
                - Evita giros bruscos o movimientos rápidos sin apoyo.
                - Evalúa el uso de bastón o ayuda técnica ocasional.
                - Consulta con fisioterapeuta o médico sobre tu marcha.
                - Revisa medicamentos que puedan afectar el equilibrio.
                """)
            else:
                st.error("🔴 Riesgo alto. Marcha inestable. Acción inmediata recomendada.")
                st.markdown("""
                - **No te muevas solo/a sin apoyo.**
                - Usa ayuda técnica (andador o bastón) bajo supervisión.
                - Programa una evaluación neurológica con un especialista.
                - Inicia fisioterapia inmediatamente.
                - Adapta tu entorno para prevenir caídas.
                - Supervisión constante durante la deambulación.
                """)

            # --- REPORTE COMBINADO ---
            report = f"""
            REPORTE DE ANÁLISIS DE MARCHA Y RIESGO DE CAÍDA
            ...
            RESULTADOS DEL ANÁLISIS DE MARCHA
            ...
            Score: {risk['score']}%
            Factores de Riesgo:
            Variabilidad aceleración: {risk['factors']['Acc_Var']:.1f}%
            Variabilidad giroscopio: {risk['factors']['Giro_Var']:.1f}%
            Movimientos súbitos: {risk['factors']['Mov_Súbitos']:.1f}%
            ...
            st.text_area("Contenido del Reporte", report, height=900)
            try:
                pdf_bytes = text_to_pdf(report)
                st.download_button(
                    label="📄 Descargar Reporte (PDF)",
                    data=pdf_bytes,
                    file_name=f"reporte_marcha_y_riesgo_{edad}años_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.pdf",
...
<div style="
    ...
    border-radius: 8px;
">
    <strong>SIAM - Sistema Integral de Análisis de Marcha</strong><br>
    © 2025 | Desarrollado por Israel E. Nieto Granados para el análisis de marcha en personas con Alzheimer.<br>
    Versión 1.0.0 | <a href="mailto:23500377@uagro.mx">Contacto</a> | <a href="#">Documentación</a>
</div>
